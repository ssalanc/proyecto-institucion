  ...
  if (!modalidades || modalidades.length === 0) {
    return (
      <div className="text-center py-12 bg-gray-50 rounded-lg">
        <p className="text-gray-500 text-lg font-bold">No hay modalidades registradas</p>
      </div>
    );
  }
            ...
            {modalidades.map((modalidad, index) => (
              <tr key={modalidad.id} className="hover:bg-gray-50 transition">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  ...
                  <button
                    onClick={() => setConfirmDelete({ show: true, id: modalidad.id })}
      ...
      {confirmDelete.show && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 text-center border border-gray-100">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-2">¿Eliminar modalidad?</h3>
            <p className="text-sm text-gray-500 mb-6">
              Esta acción puede afectar a las carreras vinculadas a esta modalidad.
            </p>
